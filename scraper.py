    ...
    try:
        for page_config in load_configs():
            try:
                ...
                mail_body = 'Error occured while running script for nepremicnine scrapper for {0}. Check what is going on!\n{1}'.format(page_config.key, str(e))
        ...
        mail_body = 'Some general error occured while running script for nepremicnine scrapper. Check what is going on!\n' + str(e)
        Mail().set_parameters_error(
                page_config.from_email,
                page_config.gmail_api_key,
                page_config.admin_mail,
